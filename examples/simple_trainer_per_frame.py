...
class Config:
    ...
    def __post_init__(self):
        ...
        self.save_steps = [self.max_steps // 3, 2 * self.max_steps // 3, self.max_steps]
...
class Runner:
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            mask = data["mask"].to(device).clamp(0, 1)  # [1, H, W]
            ...
            #     with torch.no_grad():
            #         k
            #         canvas = torch.cat([pixels, colors, colors_mask], dim=2).squeeze(0).cpu().numpy()
            #         imageio.imwrite(
            #             f"/cs/student/projects4/ml/2023/asrivast/datasets/dress2_3/upright/results_debug/run0/renders/{step}_no_rnd_bkgd_masked_clamped.png", 
            ...
            l1loss = F.l1_loss(colors_mask, pixels)
            l2loss = F.mse_loss(colors_mask, pixels)
            ssimloss = 1.0 - self.ssim(
                pixels.permute(0, 3, 1, 2), colors_mask.permute(0, 3, 1, 2)
            )
            ...
            if self.SMPL is not None and step %100 == 0:
