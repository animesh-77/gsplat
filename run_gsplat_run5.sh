...
if [ "$#" -ne 3 ]; then
    ...
    echo "Usage: run_gsplat_run5.sh <dress_name> <data_factor> <run_grounding_mask=true>"
    exit 1
fi
...
run 5 loss: ssim+L1, with mask, test_every 8, data_factor $2, ssim_lambda 0.2 , sh_degree 3, steps_scaler 1
EOL
...
if [ "$3" = true ]; then
    ...
    ################## Run DINO_SAM.py ##################
    python examples/Grounding_SAM_seg.py \
